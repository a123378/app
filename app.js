...
let state = {
  ...
  historyByDate: {} // Nested Map schema: 'YYYY-MM-DD': Array of { id, type, points, amount, timestamp, pattern }
};
...
// Update Today's Summary stats & Today's profit
function renderDashboard() {
  const todayStr = getLocalDateString();
  const todayRecords = state.historyByDate[todayStr] || [];
  ...
  // Render today's profit with format
  const formattedProfit = (todayProfit >= 0 ? '+' : '-') + '$' + Math.abs(todayProfit).toLocaleString();
  ...
  // Set supplementary metrics for today
  const todayHands = todayRecords.length;
  el.totalHandsCount.textContent = `${todayHands} 局`;
  ...
  // Calculate percentage rates relative to today's total hands (Win rate includes both self-drawn and win-card)
  const selfDrawnRate = todayHands > 0 ? Math.round((selfDrawnCount / todayHands) * 100) : 0;
  const winRate = todayHands > 0 ? Math.round(((selfDrawnCount + winCount) / todayHands) * 100) : 0;
  const discardRate = todayHands > 0 ? Math.round((discardCount / todayHands) * 100) : 0;
...
function renderDailyDetailsPage(date) {
  ...
  // Render Day History List
  el.dayHistoryList.innerHTML = '';
  
  // Render history in reverse order (newest on top) but numbered chronologically
  ...
  reversedRecords.forEach(item => {
    ...
    el.dayHistoryList.appendChild(card);
  });
}
...
function saveDirectRecord(item) {
  ...
  const todayHands = state.historyByDate[todayStr].length;
  showToast(`${emoji} 已記錄今日第 ${todayHands} 局: ${actionLabel} ${item.points} 台 (${sign} $${Math.abs(item.amount).toLocaleString()})`);
}
...
function confirmBigHandRecord() {
  ...
  const todayHands = state.historyByDate[todayStr].length;
  showToast(`🀄 已記錄今日第 ${todayHands} 局: 自摸 ${pendingBigHandItem.points} 台${patternNote} (贏 $${Math.abs(pendingBigHandItem.amount).toLocaleString()})`);
...
// Complete current session and wipe state
function resetSession() {
  if (confirm('⚠️ 警告：您確定要結束這雀並清空所有紀錄嗎？\n此動作將會清除目前的所有局數與每日戰績，無法復原！')) {
    state.historyByDate = {};
    saveStateToStorage();
    renderAll();
